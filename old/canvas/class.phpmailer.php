...
class PHPMailer {
  ...
  public function SmtpConnect() {
     ...
     * Se pueden indicar multiples SMTP hosts separados por coma
     * El port puede indicarse agregando al final :port
     * mail.domain.com:25
     * 
     * Si no se especifica el port en el host, se toma el port del atributo Port del objeto.
     * */
  ...
  public function CreateHeader() 
  {
    ...
    $result .= $this->HeaderLine('X-Mailer', 'Plataforma4 [www.commsys.com.ar]');
  ...
  public function AttachAll() 
  {
    ...
    for($i = 0; $i < count($this->attachment); $i++) 
    {
      ...
      {
      	$mime[] = sprintf("Content-ID: <%s>%s", md5($i), $this->LE);
      }
	  
      $mime[] = sprintf("X-Attachment-Id: f_%03d%s", $i, $this->LE);
  ...
  */
  ...
  public function AddEmbeddedImage($path, $cid, $name = '', $encoding = 'base64', $type = 'application/octet-stream') 
  {
    if(!is_file($path)) 
    {
      error_log("AddEmbeddedImage() Error el archivo no existe. ({$path})");	
      $this->SetError($this->Lang('file_access') . $path);
      return false;
    }
  ...
  {
  	error_log("MsgHTML() -> BaseDir='{$basedir}'");
    ...
    if(isset($images[2])) 
    {
      foreach($images[2] as $i => $url) 
      {
        ...
        if (!preg_match('/^[A-z][A-z]*:\/\//',$url)) 
        {
          ...
          error_log("MsgHTML() -> AddImage={$basedir}{$directory}{$filename} Mime={$mimeType} Filename={$filename}");
          if ( $this->AddEmbeddedImage($basedir.$directory.$filename, md5($filename), $filename, 'base64', $mimeType) ) 
          {
          	error_log("MsgHTML() -> AddImage = OK");
